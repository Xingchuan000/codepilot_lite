...
def _records(tmp_path: Path, tool_name: str, arguments: dict, token: dict):
    ...
    call = store.tool_executions.create_tool_call(turn_id=turn.turn_id, attempt_id=attempt.attempt_id, tool_name=tool_name, arguments=arguments)
    store.tool_executions.persist_tool_execution_started(call.tool_call_id, token)
...
def test_recover_completed_call_and_in_progress_message_is_idempotent(tmp_path: Path) -> None:
    ...
    message = store.messages.create_message(session_id=session.session_id, turn_id=turn.turn_id, attempt_id=attempt.attempt_id, role="assistant", status="in_progress", content="partial")
    store.messages.append_message_part(message.message_id, type="text", content="partial")
    ...
    assert store.tool_executions.get_tool_call(call.tool_call_id).status == "completed"
    assert store.tool_executions.get_tool_result_by_call(call.tool_call_id).status == "recovered_completed"
    ...
    assert any(
        ...
        and "recovered_completed" in item.content
        and "replace_range" in item.content
        and "sample.txt" in item.content
        for item in context
    )
...
def test_unknown_call_requires_user_and_retry_preserves_original_call(tmp_path: Path) -> None:
    ...
    assert store.tool_executions.get_tool_call(call.tool_call_id).status == "execution_uncertain"
    assert store.tool_executions.get_tool_result_by_call(call.tool_call_id).status == "execution_uncertain"
    assert attempt is not None
    assert attempt.turn_id == turn.turn_id
    assert store.turns.get_turn(turn.turn_id).status == "queued"
    assert sum(event.event_type == "tool_reconciled" for event in store.events.list_events(session.session_id)) == 1
...
def test_active_worker_is_not_recovered_and_created_attempt_is_rescheduled(tmp_path: Path) -> None:
    ...
    store.attempts.start_turn_attempt(
        turn.turn_id,
        attempt.attempt_id,
    ...
    assert store.attempts.get_attempt(attempt.attempt_id).status == "running"
...
def test_abort_closes_every_uncertain_call_in_turn(tmp_path: Path) -> None:
    ...
    second = store.tool_executions.create_tool_call(
    ...
    assert store.tool_executions.get_tool_call(first.tool_call_id).status == "recovery_aborted"
    assert store.tool_executions.get_tool_call(second.tool_call_id).status == "recovery_aborted"
    assert store.tool_executions.get_tool_result_by_call(first.tool_call_id).status == "recovery_aborted"
    assert store.tool_executions.get_tool_result_by_call(second.tool_call_id).status == "recovery_aborted"
...
def test_pending_approval_blocks_automatic_recovery_attempt(tmp_path: Path) -> None:
    ...
    attempt = store.attempts.create_attempt(turn_id=turn.turn_id, status="running", started_at="2024-01-01T00:00:00+00:00")
    call = store.tool_executions.create_tool_call(turn_id=turn.turn_id, attempt_id=attempt.attempt_id, tool_name="run_shell", arguments={"repo": str(tmp_path), "command": "echo hi"}, status="approval_pending")
    store.permissions.create_permission_request(
    ...
    assert store.turns.get_turn(turn.turn_id).status == "running"
...
def test_approval_after_restart_finalizes_old_tool_call_as_not_executed(tmp_path: Path) -> None:
    ...
    attempt = store.attempts.create_attempt(turn_id=turn.turn_id, status="running", started_at="2024-01-01T00:00:00+00:00")
    call = store.tool_executions.create_tool_call(turn_id=turn.turn_id, attempt_id=attempt.attempt_id, tool_name="run_shell", arguments={"command": "echo hi"}, status="approval_pending")
    store.permissions.create_permission_request(
    ...
    store.permissions.persist_permission_resolution(
    ...
    assert store.tool_executions.get_tool_call(call.tool_call_id).status == "recovered_not_executed"
    assert store.tool_executions.get_tool_result_by_call(call.tool_call_id).status == "recovered_not_executed"
...
def test_recovery_claims_only_one_interrupted_turn_at_a_time(tmp_path: Path) -> None:
    ...
    assert len(first.resumable_attempt_ids) == 1
    assert store.attempts.get_attempt(first.resumable_attempt_ids[0]).turn_id == turns[0].turn_id
    assert store.turns.get_turn(turns[1].turn_id).status == "interrupted"
