...
def test_restart_without_child_handle_marks_running_child_recovery_required(tmp_path: Path) -> None:
    ...
    assert store.sessions.get_session(child.session_id).metadata["agent_status"] == "recovery_required"
    assert restarted.list_agents(parent.session_id) == [snapshot]
    assert store.turns.get_turn(child_turn.turn_id).status == "running"
