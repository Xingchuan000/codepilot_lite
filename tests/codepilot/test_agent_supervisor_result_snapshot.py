...
def _completed_child(tmp_path: Path, *, runtime_result: str | None = "runtime summary fallback"):
    ...
    parent = store.sessions.create_session(
        project_path=repo,
        provider="openai",
        current_model="fake",
    ...
    child = store.sessions.create_session(
        project_path=repo,
        provider="openai",
        current_model="fake",
        permission_mode="manual",
...
def test_wait_reconstructs_streamed_assistant_result_when_runtime_summary_missing(tmp_path: Path) -> None:
    ...
    store.messages.append_message_part(message.message_id, type="reasoning", content="private reasoning")
    store.messages.append_message_part(message.message_id, type="text", content="Root cause: ")
    store.messages.append_message_part(message.message_id, type="text", content="VIP uses 5% instead of 10%.")
...
def test_snapshot_prefers_structured_runtime_summary_and_exposes_child_evidence(tmp_path: Path) -> None:
    ...
    store.sessions.update_session(
        child.session_id,
        metadata={
            **store.sessions.get_session(child.session_id).metadata,
    ...
    store.messages.append_message_part(message.message_id, type="text", content="Task complete.")
