...
def test_native_tool_call_persists_provider_and_internal_ids_separately(tmp_path: Path) -> None:
    ...
    messages = store.list_messages_with_parts(session_id, turn_id)
    ...
    assert store.get_tool_call(internal_tool_call_id).metadata["provider_tool_call_id"] == provider_tool_call_id
