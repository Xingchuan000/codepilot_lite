...
def _session(tmp_path: Path) -> tuple[SessionDatabase, SessionStore, str, str]:
...
def test_native_replay_keeps_provider_id_and_tool_role(tmp_path: Path) -> None:
    ...
    assistant = store.create_message(session_id=session_id, turn_id=turn_id, role="assistant", status="completed", content="")
    store.append_message_part(
        assistant.message_id,
        type="tool_call",
        content={"provider_tool_call_id": "provider-call-1", "tool_name": "read_file", "arguments": {"path": "a.py"}},
    )
    tool = store.create_message(session_id=session_id, turn_id=turn_id, role="tool", status="completed", content="ok")
    store.append_message_part(
        tool.message_id,
        type="tool_result",
        content={
    ...
    history = SessionHistory(session_id, turn_id, tmp_path, (), tuple(store.list_messages_with_parts(session_id)))
...
def test_native_replay_fails_fast_for_legacy_tool_part(tmp_path: Path) -> None:
    database, store, session_id, turn_id = _session(tmp_path)
    assistant = store.create_message(session_id=session_id, turn_id=turn_id, role="assistant", status="completed", content="")
    ...
    with pytest.raises(SessionProtocolMismatch, match="start a new session"):
