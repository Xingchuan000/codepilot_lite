...
def hydrate_session_view(store: SessionStore, session_id: str) -> AgentRunView:
    ...
    for message, parts in store.list_messages_with_parts(session_id):
        content = "\n".join(part.content if isinstance(part.content, str) else json.dumps(part.content, ensure_ascii=False) for part in parts if part.replayable)
        if not content:
        ...
        items.append(TranscriptItem(id=message.message_id, kind=kind, timestamp=message.created_at, title=message.role, body=content, status=message.status, copy_text=f"{message.role}: {content}"))
