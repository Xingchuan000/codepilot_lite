    ...
    def load_session(self, session_id: str) -> TUISession:
        ...
        runs = tuple(
            TUISessionRunRef(
                ...
                assistant_stop_reason=item.get("assistant_stop_reason"),
                delivery_kind=item.get("delivery_kind"),
                requires_evidence=item.get("requires_evidence"),
                evidence_reasons=tuple(item.get("evidence_reasons", [])),
                write_attempted=item.get("write_attempted"),
                write_executed=item.get("write_executed"),
                written_files=tuple(item.get("written_files", [])),
                changed_files=tuple(item.get("changed_files", [])),
                tests_required=item.get("tests_required"),
                diff_required=item.get("diff_required"),
                diff_checked=item.get("diff_checked"),
