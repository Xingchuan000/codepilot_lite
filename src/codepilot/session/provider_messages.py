...
def to_provider_messages(
    ...
    *,
    ...
    def provider_tool_name(name: str) -> str:
        if tool_name_map is not None and name in tool_name_map:
            return tool_name_map[name]
        return to_provider_tool_name(name)
        ...
        if message.role == "assistant":
            ...
            for part in message.parts:
                ...
                if part.type == "reasoning_replay":
                    if part.provider_format != "anthropic_thinking_blocks":
                        raise ValueError(f"Unsupported reasoning replay format: {part.provider_format}")
