...
class ContextAssembler:
    ...
    def __init__(self, database: SessionDatabase, store: SessionRepositories | None = None) -> None:
        self.store = store or SessionRepositories(database)
    ...
    def build_history(self, session_id: str, current_turn_id: str, profile=None) -> SessionHistory:
        ...
        turn = self.store.turns.get_turn(current_turn_id)
        profile = profile or resolve_model_context_profile(turn.provider_snapshot, turn.model_snapshot)
        project_path = self.store.projects.get_project(session.project_id).path
        ...
        return SessionHistory(
            ...
            memory_items=self.memory.memory_items(
                ...
                self.memory_query.build(
                    ...
                    current_turn_id=current_turn_id,
                    current_user_text=(
                        str(self.store.messages.get_user_message_for_turn(current_turn_id).content)
                        if self.store.messages.get_user_message_for_turn(current_turn_id) is not None
                        else ""
                    ),
    ...
    def _build_inherited_items(
        ...
        if policy.mode == "none":
            return ()
        fork_turn = self.store.turns.get_turn(session.forked_from_turn_id)
        if fork_turn.session_id != session.parent_session_id:
            raise ValueError("fork turn does not belong to parent session")
        parent_turns = [
            turn
            for turn in self.store.turns.list_turns(session.parent_session_id)
        ...
        if policy.mode in {"recent", "summary_recent", "full"}:
            ...
            for message, parts in self.store.messages.list_messages_with_parts(session.parent_session_id):
    ...
    def _latest_parent_summary_before(self, parent_session_id: str, fork_sequence: int) -> ContextSummaryRecord | None:
        summaries = self.store.context_summaries.list_context_summaries(parent_session_id)
        for summary in reversed(summaries):
            ...
            if summary.source_end_sequence is None and summary.turn_id is not None:
                if self.store.turns.get_turn(summary.turn_id).sequence > fork_sequence:
                    continue
            return summary
        return None


def _messages_for_turn(store: SessionRepositories, session_id: str, current_turn_sequence: int) -> tuple[tuple[MessageRecord, tuple[MessagePartRecord, ...]], ...]:
    turn_by_id = {item.turn_id: item for item in store.turns.list_turns(session_id)}
    messages = []
    for message, parts in store.messages.list_messages_with_parts(session_id):
...
def _turn_sequence(store: SessionRepositories, turn_id: str) -> int | None:
    try:
        return store.turns.get_turn(turn_id).sequence
    except LookupError:
        return None


def _branch_messages(store: SessionRepositories, session_id: str, current_turn_sequence: int) -> list[ChatMessage]:
